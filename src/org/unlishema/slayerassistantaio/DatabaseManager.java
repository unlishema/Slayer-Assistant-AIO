...
public class DatabaseManager {
	...
	public Assignment getAssignmentByAlias(final String alias) {
		...
		for (final Assignment assignment : this.assignments)
			if (assignment.hasAlias(alias.toLowerCase())) return assignment;
		return null;
	}
	...
	public Creature getCreatureByName(final String name) {
		...
		for (final Creature creature : this.creatures)
			if (creature.name.toLowerCase().equals(name.toLowerCase())) return creature;
		return null;
	}
	...
	public Master getMasterByName(final String name) {
		...
		for (final Master master : this.masters)
			if (master.name.toLowerCase().equals(name.toLowerCase())) return master;
		return null;
	}
	...
	public ArrayList<Creature> getReaperCreatures() {
		...
		for (final Reaper reaper : this.reapers)
			for (final Creature creature : reaper.creatures) creatures.add(creature);
		return creatures;
	}
	...
	public void loadDatabase() {
		...
		if (defaultsFile.exists() && defaultsFile.length() > 0) {
			...
			// TODO Add Extra Items for Task (Herbacide, Seedacide, and any others the wiki
			// didn't add)
			...
			try {
				...
				for (int i = 0; i < d.length(); i++) {
					...
					final String defaultCreature = obj.has("default-creature") ? obj.getString("default-creature")
							: null;
					final JSONArray hiddenCreatures = obj.has("hidden-creatures") ? obj.getJSONArray("hidden-creatures")
							: new JSONArray();
					...
					if (hiddenCreatures != null) for (int j = 0; j < hiddenCreatures.length(); j++)
						assignment.addHiddenCreature(hiddenCreatures.getString(j));
	...
	public void saveDatabase() throws IOException, JSONException {
		...
		if (this.imageManager.hasData()) {
			...
			try {
				final JSONObject arr = imagesFile.exists() && imagesFile.length() > 0
						? Util.loadJSONObject(imagesFile.toString())
						: new JSONObject();
		...
		if (this.assignments.size() > 0) {
			...
			try {
				final JSONArray arr = assignmentsFile.exists() && assignmentsFile.length() > 0
						? Util.loadJSONArray(assignmentsFile.toString())
						: new JSONArray();
				for (final Assignment assignment : this.assignments) {
		...
		if (this.reapers.size() > 0) {
			...
			try {
				final JSONArray arr = reapersFile.exists() && reapersFile.length() > 0
						? Util.loadJSONArray(reapersFile.toString())
						: new JSONArray();
				for (final Reaper reaper : this.reapers) {
					for (int i = 0; i < arr.length(); i++) {
						final JSONObject obj = arr.getJSONObject(i);
		...
		if (this.creatures.size() > 0) {
			...
			final JSONArray arr = creaturesFile.exists() && creaturesFile.length() > 0
					? Util.loadJSONArray(creaturesFile.toString())
					: new JSONArray();
			for (final Creature creature : this.creatures) {
				for (int i = 0; i < arr.length(); i++) {
					final JSONObject obj = arr.getJSONObject(i);
		...
		if (this.masters.size() > 0) {
			...
			try {
				final JSONArray arr = mastersFile.exists() && mastersFile.length() > 0
						? Util.loadJSONArray(mastersFile.toString())
						: new JSONArray();
				for (final Master master : this.masters) {
					for (int i = 0; i < arr.length(); i++) {
						final JSONObject obj = arr.getJSONObject(i);
  ...
  // @formatter:on
	...
	public void updateRS3Indexes() {
		...
		if (!rs3File.exists()
				|| (rs3File.exists() && rs3File.lastModified() <= System.currentTimeMillis() - Util.OLDER_THAN_TIME)) {
	...
	public void updateTasks() {
		...
		if (!assignmentWikiFile.exists() || (assignmentWikiFile.exists()
				&& assignmentWikiFile.lastModified() <= System.currentTimeMillis() - Util.OLDER_THAN_TIME)) {
	...
	public void updateReapers() {
		...
		if (!reaperWikiFile.exists() || (reaperWikiFile.exists()
				&& reaperWikiFile.lastModified() <= System.currentTimeMillis() - Util.OLDER_THAN_TIME)) {
	...
	public void updateAssignment(final Assignment assignment) {
		...
		if (!assignmentWikiFile.exists() || (assignmentWikiFile.exists()
			...
			Util.downloadJSON(
					"https://runescape.wiki/api.php?action=parse&format=json&formatversion=2&prop=text&redirects=1&page="
							+ URLEncoder.encode(assignment.index, StandardCharsets.UTF_8) + "_(Slayer_assignment)",
					assignmentWikiFile);
		}
	...
	public void updateCreature(final Creature creature) {
		...
		// HARDCODE Fix because Fenris wolf (Sk�ll) page doesn't exist and Ghost
		// (Shattered Worlds) is a
		// mess
		if (creature.name.equals("Fenris wolf (Sk�ll)")) return; // Removed from game
		...
		if (!creatureWikiFile.exists() || (creatureWikiFile.exists()
				&& creatureWikiFile.lastModified() <= System.currentTimeMillis() - Util.OLDER_THAN_TIME)) {
			if (Util.DEBUG) System.out.println("[Creature] Updating: " + creatureWikiFile);
			Util.downloadJSON(
					"https://runescape.wiki/api.php?action=parse&format=json&formatversion=2&prop=text&redirects=1&page="
							+ URLEncoder.encode(creature.name, StandardCharsets.UTF_8),
					creatureWikiFile);
		}
	...
	private void extractIndexes() {
		try {
			...
			// HARDCODE Fix to skip creatures that are no longer relevant or that's causing
			// issues
			final String[] skipCreatures = new String[] {
					"Dwarves", "Monkeys", "Phantoms", "Pigs", "Polypore creatures", "Spiritual rangers",
					"Zarosian creatures"
			};
			...
			if (this.assignments.size() == 0) {
				...
				final String[] indexesToAddAlias = new String[] {
						"Abyssal Demons", "Acheron mammoths", "Ankou", "Aviansies", "Camel warriors", "Cave slimes",
						"Cockatrice", "Cres's creations", "Creatures of the Lost Grove", "Fleshcrawlers", "Fungal magi",
						"Glacors", "Goraks", "Kal'gerion demons", "Kalphite", "Kurask", "Nightmare creatures",
						"Ripper demons", "Risen ghosts", "Rockslugs", "Rune dragons", "Soul devourers", "Suqahs",
						"Tormented demons"
				};
				final String[] aliasToAdd = new String[] {
						"Abyssal demons", "Acheron mammoth", "Ankous", "Aviansie", "Camel Warrior", "Cave slime",
						"Cockatrices", "Automatons", "Lost Grove creatures", "Flesh Crawlers", "Fungal mages", "Glacor",
						"Gorak", "Kal'gerion demon", "Kalphites", "Kurasks", "Nightmare", "Ripper Demon", "Risen ghost",
						"Rock slugs", "Rune dragon", "Soul devourer", "Suqah", "Tormented demon"
				};
				// HARDCODE Fix to replace indexes as aliases
				final String[] indexesToReplaceAsAlias = new String[] {
						"Corrupted dust devils", "Corrupted kalphites", "Corrupted lizards", "Corrupted scarabs",
						"Corrupted scorpions", "Corrupted worker", "Greater demon berserkers",
						"Greater demon ash lords", "Crocodile akh", "Feline akh", "Gorilla akh", "Imperial guard akh",
						"Imperial mage akh", "Imperial ranger akh", "Imperial warrior akh", "Salawa akh", "Scarab akh",
						"Volcanic creatures"
				};
				final String[] aliasesToReplaceWith = new String[] {
						"Corrupted creatures", "Corrupted creatures", "Corrupted creatures", "Corrupted creatures",
						"Corrupted creatures", "Corrupted creatures", "Greater demons", "Greater demons",
						"Soul devourers", "Soul devourers", "Soul devourers", "Soul devourers", "Soul devourers",
						"Soul devourers", "Soul devourers", "Soul devourers", "Soul devourers", "TzHaar"
				};
				...
				for (final String index : keys) {
					...
					if (Util.arrayContains(skipCreatures, index)
							|| Util.arrayContains(indexesToReplaceAsAlias, index)) {
						...
						// HARDCODE Fix to add aliases to assignments that need it (To align indexes and
						// wiki)
						if (Util.arrayContains(indexesToAddAlias, index)) {
							...
							if (Util.DEBUG_VERBOSE) System.out
									.println("[Fresh Start] Adding Fixed Alias: " + index + " => " + aliasToAdd[i]);
						}
					}
				}
				...
				for (final String index : keys) {
					if (Util.arrayContains(indexesToReplaceAsAlias, index)) {
						...
						if (aliasAssignment != null) {
							...
							if (Util.DEBUG_VERBOSE) System.out.println("[Fresh Start] Including Index as Alias: "
									+ index + " => " + aliasesToReplaceWith[i]);
						}
					}
				}

				// HARDCODE Fix for "Greater demon berserkers and ash lords" (index and wiki
				// don't match up)
				final Assignment greaterDemonAssignment = this
						.getAssignmentByAlias("Greater demon berserkers and ash lords");
				if (greaterDemonAssignment != null) {
					if (!greaterDemonAssignment.hasAlias("Greater demon ash lords")) {
						...
						if (Util.DEBUG_VERBOSE) System.out.println("[Fresh Start] Adding Fixed Aliases: "
								+ greaterDemonAssignment.index + " => Greater demon ash lords");
					}
					if (!greaterDemonAssignment.hasAlias("Greater demon berserkers")) {
						greaterDemonAssignment.addAlias("Greater demon berserkers");
						System.out.println("[Fresh Start] Adding Fixed Aliases: " + greaterDemonAssignment.index
			...
			// Add Missing Indexes
			for (final String index : keys)
				if (this.getAssignmentByAlias(index) == null && !Util.arrayContains(skipCreatures, index)) {
	...
	private void extractAssignments() {
		try {
			...
			for (int i = 1; i < assignmentsTable.size(); i++) {
				...
				// HARDCODE Fix to adjust for cluster tasks and other messed up assignments on
				// wiki
				...
				final String[] masterList = new String[] {
						"Turael", "Jacquelyn", "Vannaka", "The Raptor", "Mazchna", "Chaeldar", "Sumona", "Duradel",
						"Kuradal", "Morvran", "Laniakea", "Mandrith"
				};
			...
			for (int i = 2; i < tableRows.size(); i++) {
				...
				if (linksReaper.size() > 0) {
					...
					if (creatureName.equals("Dagannoth Kings")) {
						...
						final String[] fixNames = new String[] {
								"Legio Primus", "Legio Secundus", "Legio Tertius", "Legio Quartus", "Legio Quintus",
								"Legio Sextus"
						};
						for (final String fixName : fixNames) {
							Creature creature = this.getCreatureByName(fixName);
						...
						final String[] fixNames = new String[] {
								"Ahrim the Blighted", "Akrisae the Doomed", "Dharok the Wretched",
								"Guthan the Infested", "Karil the Tainted", "Linza the Disgraced",
						...
						final String[] fixNames = new String[] {
								...
								"Torag the Corrupted", "Verac the Defiled", "Karil the Tainted"
						};
						for (final String fixName : fixNames) {
							Creature creature = this.getCreatureByName(fixName);
							if (creature == null) {
		...
		try {
			...
			for (int i = 1; i < creaturesTable.size(); i++) {
				...
				if (creature == null) {
					...
					this.creatures.add(creature);
					if (assignment.defaultCreature == null || assignment.defaultCreature.isEmpty())
						assignment.defaultCreature = creature.name;
				}
	...
	private void extractCreature(final Creature creature) {
		...
		try {
			...
			final Elements variantElements = hasMultiple
					? tableElement.getElementsByClass("infobox-buttons").first().children()
					: null;
			...
			for (int i = 0; i < variantCount; i++) {
				...
				final CreatureVariant variant = new CreatureVariant(
						hasMultiple ? variantNameElement.html() : "Default");
				...
				variant.image = Util.extractImageResource(imageHtml.getElementsByTag("img").first(),
						"./images/creatures/" + creature.name + "#" + variant.name,
						imageHtml.getElementsByTag("img").first().attr("title") + ".png");

				// Extract Exp Stats
				variant.experience = Float.parseFloat(
						Util.extractCreatureResource(doc, "experience", index, hasMultiple).replaceAll(",", ""));
				variant.lifepointExp = Float
						.parseFloat(Util.extractCreatureResource(doc, "hpxp", index, hasMultiple).replaceAll(",", ""));
				variant.slayerExp = Float.parseFloat(
						Util.extractCreatureResource(doc, "slayxp", index, hasMultiple).replaceAll(",", ""));
				...
				variant.maxRanged = Util.extractCreatureResource(doc, "max_ranged", index, hasMultiple).replaceAll(",",
						"");
				variant.maxNecro = Util.extractCreatureResource(doc, "max_necromancy", index, hasMultiple)
						.replaceAll(",", "");
				variant.maxSpec = Util.extractCreatureResource(doc, "max_spec", index, hasMultiple).replaceAll(",", "");

				// Extract Susceptible
				final Document susceptibilityHtml = Jsoup
						.parse(Util.extractCreatureResource(doc, "susceptibility", index, hasMultiple));
				for (final Element susceptibilityElement : susceptibilityHtml.getElementsByTag("a")) {
					...
					final String imgName = src.substring(
							src.indexOf(isThumb ? "/images/thumb/" : "/images/") + (isThumb ? 14 : 8),
							src.indexOf(isThumb ? "/25px-" : "?"));
					Main.dbm.imageManager.addSusceptible(imgName,
							"https://runescape.wiki/" + imgUrlElement.attr("src"));

					variant.addSusceptibility(
							Util.extractLinkImageResource(susceptibilityElement, "./images/susceptible/"));
				}

				// Extract Weakness
				final Document weaknessHtml = Jsoup
						.parse(Util.extractCreatureResource(doc, "weakness", index, hasMultiple));
				for (final Element weaknessElement : weaknessHtml.getElementsByTag("a")) {
					...
					final boolean isThumb = srcset != null && !srcset.isEmpty();
					final String imgName = src.substring(
							src.indexOf(isThumb ? "/images/thumb/" : "/imagea/") + (isThumb ? 14 : 8),
							src.indexOf(isThumb ? "/25px-" : "?"));
