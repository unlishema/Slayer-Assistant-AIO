...
public class Reaper {
	...
	public Reaper(final String index) {
		this(index, null, "");
	}

	public Reaper(final String index, final String defaultCreature) {
		this(index, defaultCreature, "");
	}

	public Reaper(final String index, final String defaultCreature, final String comment) {
		this.index = index;
		this.defaultCreature = defaultCreature;
		this.comment = comment;
	}

	public Reaper(final JSONObject obj) throws JSONException {
	...
	public JSONObject getDefaultsJSONObject() throws JSONException { return this.getJSONObject(true); }

	public JSONObject getJSONObject() throws JSONException { return this.getJSONObject(false); }
	...
	public void loadJSONObject(final JSONObject obj) throws JSONException {
		...
		if (hiddenCreatures != null)
			for (int i = 0; i < hiddenCreatures.length(); i++) this.addHiddenCreature(hiddenCreatures.getString(i));
		...
		if (creatures != null) for (int i = 0; i < creatures.length(); i++)
			this.addCreature(Main.dbm.getCreatureByName(creatures.getString(i)));

		final JSONArray masters = obj.getJSONArray("masters");
		if (masters != null)
			for (int i = 0; i < masters.length(); i++) this.addMaster(Main.dbm.getMasterByName(masters.getString(i)));
	}
	...
	public boolean equals(final Object o) {
		return o instanceof Reaper ? ((Reaper) o).index.equals(this.index) : false;
	}
