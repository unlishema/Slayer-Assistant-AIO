...
public class SATasksPanel extends JPanel {
		...
		// ======== tabbedPaneAssignments ========
		{
			...
			final JPanel panelSlayerAssignments = new JPanel();
			{
				...
				final JPanel panelUpdate = new JPanel();
				{
					...
					panelUpdate.add(labelUpdate, new GridBagConstraints(0, 0, 2, 1, 0.0, 0.0, GridBagConstraints.CENTER,
						...
						public void actionPerformed(final ActionEvent ae) {
							new Thread() {
								...
								public void run() {
									...
									Util.updateProgressDialog(1);
									Main.dbm.updateTasks();
									Util.updateProgressDialog(2);
							...
							Util.showProgressDialog("Updating Indexes", 2);
						}

					});
					panelUpdate.add(buttonUpdateIndex, new GridBagConstraints(0, 1, 1, 1, 0.0, 0.0,
					...
					panelUpdate.add(buttonUpdateAssignments, new GridBagConstraints(1, 1, 1, 1, 0.0, 0.0,
					...
					panelUpdate.add(buttonUpdateMisc, new GridBagConstraints(0, 2, 1, 1, 0.0, 0.0,
							GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 5, 5), 0, 0));
					...
					panelUpdate.add(buttonUpdateCreatures, new GridBagConstraints(1, 2, 1, 1, 0.0, 0.0,
							GridBagConstraints.CENTER, GridBagConstraints.BOTH, new Insets(0, 0, 5, 0), 0, 0));
				}
				...
				final JScrollPane scrollPaneSlayerAssignments = new JScrollPane();
				{
					...
					this.listSlayerAssignments.addListSelectionListener(new ListSelectionListener() {
						...
						public void valueChanged(final ListSelectionEvent lse) {
							Main.frame.panelTasks
									.selectAssignment(Main.frame.panelTasks.listSlayerAssignments.getSelectedValue());
						}
					});
			...
			final JPanel panelReaperAssignments = new JPanel();
			{
				...
				final JScrollPane scrollPaneReaperAssignments = new JScrollPane();
				{
					...
					this.listReaperAssignments.addListSelectionListener(new ListSelectionListener() {
						...
						public void valueChanged(final ListSelectionEvent lse) {
							Main.frame.panelTasks
									.selectReaper(Main.frame.panelTasks.listReaperAssignments.getSelectedValue());
						}
					});
	...
	public void selectReaper(final String index) {
		...
		for (final String alias : Util.sortArrayList(cr.aliases))
			Main.frame.panelAssignment.listAliasModel.addElement(alias);
		final ArrayList<String> hiddenCreatures = Util.sortArrayList(cr.hiddenCreatures);
		for (final String hiddenCreature : hiddenCreatures)
			Main.frame.panelAssignment.listHiddenCreaturesModel.addElement(hiddenCreature);
		for (final Master master : Util.sortArrayList(cr.masters))
			Main.frame.panelAssignment.listMastersModel.addElement(master.name);
		for (final String equipment : Util.sortArrayList(cr.equipment))
			Main.frame.panelAssignment.listEquipmentModel.addElement(equipment);
		for (final String location : Util.sortArrayList(cr.locations))
			Main.frame.panelAssignment.listLocationsModel.addElement(location);
		...
		// Select the currently selected creature, default creature or the first
		// creature in list
		...
		} else if (Main.frame.panelAssignment.listCreaturesModel.size() > 0)
			Main.frame.panelAssignment.listCreatures.setSelectedIndex(0);
	}
	...
	public void selectAssignment(final String index) {
		...
		for (final String alias : Util.sortArrayList(ca.aliases))
			Main.frame.panelAssignment.listAliasModel.addElement(alias);
		final ArrayList<String> hiddenCreatures = Util.sortArrayList(ca.hiddenCreatures);
		for (final String hiddenCreature : hiddenCreatures)
			Main.frame.panelAssignment.listHiddenCreaturesModel.addElement(hiddenCreature);
		for (final Master master : Util.sortArrayList(ca.masters))
			Main.frame.panelAssignment.listMastersModel.addElement(master.name);
		for (final String equipment : Util.sortArrayList(ca.equipment))
			Main.frame.panelAssignment.listEquipmentModel.addElement(equipment);
		for (final String location : Util.sortArrayList(ca.locations))
			Main.frame.panelAssignment.listLocationsModel.addElement(location);
		for (final Creature creature : Util.sortArrayList(ca.creatures))
			Main.frame.panelAssignment.listCreaturesModel.addElement(creature.name);

		// Setup Counts to help with editing
		final ArrayList<String> defaultCreatures = Util.buildDefaultCreaturesUniqueArrayList(ca.creatures, null,
				ca.hiddenCreatures);
		Main.frame.panelAssignment.labelDefaultCount.setText("" + defaultCreatures.size());
		Main.frame.panelAssignment.labelHiddenCount.setText("" + hiddenCreatures.size());
