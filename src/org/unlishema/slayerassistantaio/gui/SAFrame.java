...
public class SAFrame extends JFrame {
		...
		addWindowListener(new WindowListener() {
			...
			public void windowClosing(final WindowEvent we) {
				if (Main.databaseEdited) {
					...
					final int result = JOptionPane.showOptionDialog(Main.frame,
							"Would you like to save the database before closing?", "Closing Database",
				...
				final JButton buttonSaveDatabase = new JButton("Save Database");
				{
					...
					buttonSaveDatabase.addActionListener(new ActionListener() {
						...
						public void actionPerformed(final ActionEvent ae) {
							try {
								Main.dbm.saveDatabase();
								JOptionPane.showMessageDialog(Main.frame, "The database has been saved!",
				...
				final JButton buttonLoadDatabase = new JButton("Load Database");
				{
					...
					buttonLoadDatabase.addActionListener(new ActionListener() {
						...
						public void actionPerformed(final ActionEvent ae) {
							...
							JOptionPane.showMessageDialog(Main.frame, "The database has been loaded!",
	...
	public void reloadAssignments() {
		...
		// Setup the counts for slayer assignments
		{
			...
			final File assignmentsDir = new File("database/wiki/assignments"),
					creaturesDir = new File("database/wiki/creatures"), imagesDir = new File("images/creatures");
			Main.frame.labelMastersCount.setText(Main.dbm.masters.size() + "");
			Main.frame.labelReapersCount.setText(Main.dbm.reapers.size() + "");
			Main.frame.labelAssignmentsCount.setText(Main.dbm.assignments.size() + "/"
					+ (assignmentsDir.isDirectory() ? assignmentsDir.list().length : "?"));
			Main.frame.labelCreaturesCount.setText(
					Main.dbm.creatures.size() + "/" + (creaturesDir.isDirectory() ? creaturesDir.list().length : "?"));
			...
			Main.frame.labelVariantsCount
					.setText((variantsCount == 0 ? "?" : variantsCount) + "/" + (imagesCount == 0 ? "?" : imagesCount));
		}
		...
		if (Main.frame.panelTasks.tabbedPaneAssignments.getSelectedIndex() == 0) {
			...
			if (Main.dbm.assignments.size() > 0)
				for (final Assignment assignment : Util.sortArrayList(Main.dbm.assignments))
			...
			else if (Main.frame.panelTasks.listSlayerAssignmentsModel.size() > 0)
				Main.frame.panelTasks.listSlayerAssignments.setSelectedIndex(0);
		} else {
			...
			else if (Main.frame.panelTasks.listReaperAssignmentsModel.size() > 0)
				Main.frame.panelTasks.listReaperAssignments.setSelectedIndex(0);
		}
	}
}
